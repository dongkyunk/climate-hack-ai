...
class VQVAE_Condition(nn.Module):
    def __init__(
            self,
            in_channel=3,
            channel=128,
        ...
        self.enc_b = Encoder(in_channel, channel, n_res_block, n_res_channel, stride=4)
        self.enc_b_2 = Encoder(in_channel, channel, n_res_block, n_res_channel, stride=4)
        self.enc_t = Encoder(channel, channel, n_res_block, n_res_channel, stride=2)
        self.quantize_conv_t = nn.Conv3d(channel, n_embed, 1)
        self.quantize_conv_b = nn.Conv3d(channel, n_embed, 1)

    def forward(self, input):
        enc_b = self.enc_b(input)
        enc_b_2 = self.enc_b_2(input)
        enc_t = self.enc_t(enc_b)
        quant_t = self.quantize_conv_t(enc_t).permute(0, 2, 3, 4, 1)
        quant_b = self.quantize_conv_b(enc_b_2).permute(0, 2, 3, 4, 1)
